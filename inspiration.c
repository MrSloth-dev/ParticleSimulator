...
gs_global u8 *g_world_process_update_structure = {
    0}; // Every particle has id associated with it? Jeezuz...
...
const char *v_src =
    GL_VERSION_STR "precision mediump float;\n"
                   ...
                   "	texCoord = vec2(a_texCoord.x, bool(u_flip_y) ? 1.0 - "
                   "a_texCoord.y : a_texCoord.y);\n"
                   "}";
...
gs_vec2 calculate_mouse_position() {
  ...
  return (gs_vec2){x_scale * (f32)g_texture_width,
                   y_scale * (f32)g_texture_height};
}
...
b32 in_bounds(s32 x, s32 y) {
  if (x < 0 || x > (g_texture_width - 1) || y < 0 || y > (g_texture_height - 1))
    return false;
  return true;
}
...
b32 is_empty(s32 x, s32 y) {
  return (in_bounds(x, y) &&
          g_world_particle_data[compute_idx(x, y)].id == mat_id_empty);
}
...
b32 is_in_liquid(s32 x, s32 y, s32 *lx, s32 *ly) {
  if (in_bounds(x, y) && (get_particle_at(x, y).id == mat_id_water ||
                          get_particle_at(x, y).id == mat_id_oil)) {
    *lx = x;
    *ly = y;
    return true;
  }
  if (in_bounds(x, y - 1) && (get_particle_at(x, y - 1).id == mat_id_water ||
                              get_particle_at(x, y - 1).id == mat_id_oil)) {
    *lx = x;
    *ly = y - 1;
    return true;
  }
  ...
  if (in_bounds(x - 1, y) && (get_particle_at(x - 1, y).id == mat_id_water ||
                              get_particle_at(x - 1, y).id == mat_id_oil)) {
    *lx = x - 1;
    *ly = y;
    return true;
  }
  if (in_bounds(x - 1, y - 1) &&
      (get_particle_at(x - 1, y - 1).id == mat_id_water ||
       get_particle_at(x - 1, y - 1).id == mat_id_oil)) {
    *lx = x - 1;
    *ly = y - 1;
    return true;
  }
...
void drop_file_callback(void *platform_window, s32 count,
  ...
  for (s32 i = 0; i < count; ++i) {
    ...
    gs_util_get_file_extension(temp_file_extension_buffer,
                               sizeof(temp_file_extension_buffer),
                               file_paths[0]);
    if (gs_string_compare_equal(temp_file_extension_buffer, "png") ||
      ...
      gs_util_load_texture_data_from_file(file_paths[i], &_w, &_h, &_n,
                                          &texture_data, false);
      _n = 3;

      // Not sure what the format should be, so this is ...blah. Need to find a
      // way to determine this beforehand.
      u8 *data = (u8 *)texture_data;

      s32 sx = (g_texture_width - _w) / 2;
      s32 sy = (g_texture_height - _h) / 2;

      // Now we need to process the data and place it into our particle/color
      // buffers
      for (u32 h = 0; h < _h; ++h) {
        for (u32 w = 0; w < _w; ++w) {
          color_t c = {data[(h * _w + w) * _n + 0], data[(h * _w + w) * _n + 1],
                       data[(h * _w + w) * _n + 2], 255};
...
// Here, we'll initialize all of our application data, which in this case is our
// graphics resources
void app_init() {
  // Construct command buffer (the command buffer is used to allow for immediate
  // drawing at any point in our program)
  g_cb = gs_command_buffer_new();

  // Create shader
  g_shader = gs_graphics_shader_create(&(gs_graphics_shader_desc_t){
      .sources =
  ...
  u_flip_y = gs_graphics_uniform_create(&(gs_graphics_uniform_desc_t){
      .name = "u_flip_y",
      .layout = &(gs_graphics_uniform_layout_desc_t){
  ...
  g_vbo = gs_graphics_vertex_buffer_create(&(gs_graphics_vertex_buffer_desc_t){
      .data = v_data, .size = sizeof(v_data)});

  // Construct index buffer
  g_ibo = gs_graphics_index_buffer_create(&(gs_graphics_index_buffer_desc_t){
      .data = i_data, .size = sizeof(i_data)});

  // Construct world data (for now, it'll just be the size of the screen)
  g_world_particle_data =
      gs_malloc(g_texture_width * g_texture_height * sizeof(particle_t));

  // Construct texture buffer data
  g_texture_buffer =
      gs_malloc(g_texture_width * g_texture_height * sizeof(color_t));

  g_ui_buffer = gs_malloc(g_texture_width * g_texture_height * sizeof(color_t));

  // Set buffers to 0
  memset(g_texture_buffer, 0,
         g_texture_width * g_texture_height * sizeof(color_t));
  ...
  g_rp = gs_graphics_render_pass_create(&(gs_graphics_render_pass_desc_t){
  ...
  // Pipeline
  g_pip = gs_graphics_pipeline_create(&(gs_graphics_pipeline_desc_t){
      .raster = {.shader = g_shader},
      .blend = {.func = GS_GRAPHICS_BLEND_EQUATION_ADD,
          ...
          .attrs =
              (gs_graphics_vertex_attribute_desc_t[]){
                  {.format = GS_GRAPHICS_VERTEX_ATTRIBUTE_FLOAT2,
                   .name = "a_position"}, // Named attribute required for lower
                                          // GL versions / ES2 / ES3
                  {.format = GS_GRAPHICS_VERTEX_ATTRIBUTE_FLOAT2,
                   .name = "a_texCoord"} // Named attribute required for lower
                                         // GL versions / ES2 / ES3
              },
  ...
  // platform->set_dropped_files_callback(platform->main_window(),
  // &drop_file_callback);
}
...
void app_shutdown() { gs_println("Goodbye, Gunslinger."); }
...
void update_input(gs_command_buffer_t *cb) {
  ...
  if (gs_platform_key_pressed(GS_KEYCODE_C)) {
    memset(g_texture_buffer, 0,
           sizeof(color_t) * g_texture_width * g_texture_height);
    memset(g_world_particle_data, 0,
           sizeof(particle_t) * g_texture_width * g_texture_height);
  }
  ...
  if (gs_platform_mouse_down(GS_MOUSE_RBUTTON)) {
    ...
    f32 mp_y = gs_clamp(mp.y, 0.f, (f32)g_texture_height - 1.f);
    u32 max_idx = (g_texture_width * g_texture_height) - 1;
    const f32 _R = g_selection_radius;
...
void update_particle_sim(gs_command_buffer_t *cb) {
  ...
  // Note(John): We update "bottom up", since all the data is edited "in place".
  // Double buffering all data would fix this 	issue, however it requires double
  // all of the data.
  for (u32 y = g_texture_height - 1; y > 0; --y) {
    for (u32 x = ran ? 0 : g_texture_width - 1;
         ran ? x < g_texture_width : x > 0; ran ? ++x : --x) {
      ...
      // Update particle's lifetime (I guess just use frames)? Or should I have
      // sublife?
  ...
  // Can remove this loop later on by keeping update structure and setting that
  // for the particle as it moves, then at the end of frame just memsetting the
  // entire structure to 0.
  for (u32 y = g_texture_height - 1; y > 0; --y) {
    for (u32 x = ran ? 0 : g_texture_width - 1;
         ran ? x < g_texture_width : x > 0; ran ? ++x : --x) {
      // Set particle's update to false for next frame
      g_world_particle_data[compute_idx(x, y)].has_been_updated_this_frame =
          false;
    }
  }
}

void draw_glyph_at(font_t *f, color_t *buffer, s32 x, s32 y, char c,
                   color_t col) {
  u8 *font_data = (u8 *)f->data;
  font_glyph_t g = get_glyph(f, c);

  // How to accurately place? I have width and height of glyph in texture, but
  // need to convert this to RGBA data for ui buffer
  for (s32 h = 0; h < g.height; ++h) {
    for (s32 w = 0; w < g.width; ++w) {
      s32 _w = w + g.x;
      s32 _h = h + g.y;
      u8 a = font_data[(_h * f->width + _w) * f->num_comps + 0] == 0 ? 0 : 255;
      color_t c = {font_data[(_h * f->width + _w) * f->num_comps + 0],
                   font_data[(_h * f->width + _w) * f->num_comps + 1],
                   font_data[(_h * f->width + _w) * f->num_comps + 2], a};
...
void draw_string_at(font_t *f, color_t *buffer, s32 x, s32 y, const char *str,
                    usize len, color_t col) {
  u8 *font_data = (u8 *)f->data;
  for (u32 i = 0; i < len; ++i) {
...
b32 in_rect(gs_vec2 p, gs_vec2 ro, gs_vec2 rd) {
  if (p.x < ro.x || p.x > ro.x + rd.x || p.y < ro.y || p.y > ro.y + rd.y)
    return false;
  return true;
}
...
#define __gui_interaction(x, y, w, h, c, str, id)                              \
  do {                                                                         \
    if ((id) == g_material_selection) {                                        \
      const s32 b = 2;                                                         \
      gui_rect(g_ui_buffer, x - b / 2, y - b / 2, w + b, h + b,                \
               (color_t){200, 150, 20, 255});                                  \
    }                                                                          \
    gs_vec2 mp = calculate_mouse_position();                                   \
    if (in_rect(mp, (gs_vec2){(x), (y)}, (gs_vec2){(w), (h)})) {               \
      ...
      draw_string_at(&g_font, g_ui_buffer,                                     \
                     g_texture_width / 2 + 1 - (sizeof(str) * 5) / 2, 20 - 1,  \
                     _str, sizeof(_str), s_col);                               \
      draw_string_at(&g_font, g_ui_buffer,                                     \
                     g_texture_width / 2 - (sizeof(str) * 5) / 2, 20, _str,    \
                     sizeof(_str), col);                                       \
    }                                                                          \
...
b32 update_ui(gs_command_buffer_t *cb) {
  ...
  if (g_show_material_selection_panel) {
    ...
    // Sand Selection
    __gui_interaction(g_texture_width - xoff, base + offset * 0, 10, 10,
                      mat_col_sand, "Sand", mat_sel_sand);
    __gui_interaction(g_texture_width - xoff, base + offset * 1, 10, 10,
                      mat_col_water, "Water", mat_sel_water);
    __gui_interaction(g_texture_width - xoff, base + offset * 2, 10, 10,
                      mat_col_smoke, "Smoke", mat_sel_smoke);
    __gui_interaction(g_texture_width - xoff, base + offset * 3, 10, 10,
                      mat_col_fire, "Fire", mat_sel_fire);
    __gui_interaction(g_texture_width - xoff, base + offset * 4, 10, 10,
                      mat_col_steam, "Steam", mat_sel_steam);
    __gui_interaction(g_texture_width - xoff, base + offset * 5, 10, 10,
                      mat_col_oil, "Oil", mat_sel_oil);
    __gui_interaction(g_texture_width - xoff, base + offset * 6, 10, 10,
                      mat_col_salt, "Salt", mat_sel_salt);
    __gui_interaction(g_texture_width - xoff, base + offset * 7, 10, 10,
                      mat_col_wood, "Wood", mat_sel_wood);
    __gui_interaction(g_texture_width - xoff, base + offset * 8, 10, 10,
                      mat_col_stone, "Stone", mat_sel_stone);
    __gui_interaction(g_texture_width - xoff, base + offset * 9, 10, 10,
                      mat_col_lava, "Lava", mat_sel_lava);
    __gui_interaction(g_texture_width - xoff, base + offset * 10, 10, 10,
                      mat_col_gunpowder, "GunPowder", mat_sel_gunpowder);
    __gui_interaction(g_texture_width - xoff, base + offset * 11, 10, 10,
                      mat_col_acid, "Acid", mat_sel_acid);
  }

  if (g_show_frame_count) {
    char frame_time_str[256];
    gs_snprintf(frame_time_str, sizeof(frame_time_str), "frame: %.2f ms",
                gs_engine_subsystem(platform)->time.frame);
    draw_string_at(&g_font, g_ui_buffer, 10, 10, frame_time_str,
                   strlen(frame_time_str), (color_t){255, 255, 255, 255});

    char sim_state_str[256];
    gs_snprintf(sim_state_str, sizeof(sim_state_str), "state: %s",
                g_run_simulation ? "running" : "paused");
    draw_string_at(&g_font, g_ui_buffer, 10, 20, sim_state_str,
                   strlen(sim_state_str), (color_t){255, 255, 255, 255});
  }

  // Draw circle around mouse pointer
  s32 _R = g_selection_radius;
  circleBres((s32)mp.x, (s32)mp.y, _R);

  // Request to upload our updated texture data to GPU
  ...
  desc.num_mips = 0;
  desc.width = g_texture_width;
  desc.height = g_texture_height;
  desc.data = g_ui_buffer;
  gs_graphics_texture_request_update(cb, g_tex_ui, &desc);

  return interaction;
}

void render_scene(gs_command_buffer_t *cb) {
  ...
  desc.width = g_texture_width;
  desc.height = g_texture_height;
  desc.data = g_texture_buffer;
  gs_graphics_texture_request_update(cb, g_tex, &desc);
  ...
  // Render pass action for clearing the screen
  {
    gs_graphics_clear_desc_t clear =
        (gs_graphics_clear_desc_t){.actions = &(gs_graphics_clear_action_t){
                                       .color = {0.1f, 0.1f, 0.1f, 1.f}}};

    // Uniform buffer array
    gs_graphics_bind_uniform_desc_t uniforms[] = {
        (gs_graphics_bind_uniform_desc_t){
            .uniform = u_tex, .data = &g_tex, .binding = 0},
        (gs_graphics_bind_uniform_desc_t){.uniform = u_flip_y,
                                          .data = &flip_y}};
    ...
    gs_graphics_bind_pipeline(cb, g_pip);
    gs_graphics_set_viewport(cb, 0, 0, (u32)(g_texture_width),
                             (u32)(g_texture_height));
  ...
  // Bind frame buffer for post processing
  {
    // Brightness pass
    {
      bright_filter_pass_parameters_t params =
          (bright_filter_pass_parameters_t){g_rt};
    ...
    // Blur pass
    {
      blur_pass_parameters_t params =
          (blur_pass_parameters_t){g_bright_pass.data.rt};
    ...
    {
      composite_pass_parameters_t params = (composite_pass_parameters_t){
          g_rt, g_blur_pass.data.blur_render_target_b};
  ...
  // Back buffer Presentation
  {
    ...
    gs_graphics_bind_pipeline(cb, g_pip);
    gs_graphics_set_viewport(cb, 0, 0, (u32)(fbs.x), (u32)(fbs.y));
    gs_graphics_clear(cb, &clear);
    ...
    tbind = (gs_graphics_bind_desc_t){
        .uniforms = {.desc = &(gs_graphics_bind_uniform_desc_t){
                         .uniform = u_tex, .data = &g_tex_ui, .binding = 0}}};
    gs_graphics_apply_bindings(cb, &tbind);
    gs_graphics_draw(cb, &(gs_graphics_draw_desc_t){.start = 0, .count = 6});
  ...
  gs_graphics_submit_command_buffer(&g_cb);
}
...
void update_salt(u32 x, u32 y) {
  ...
  // Just check if you can move directly beneath you. If not, then reset your
  // velocity. God, this is going to blow. if (in_bounds(x, y + 1) &&
  // !is_empty(x, y + 1) && get_particle_at(x, y + 1).id != mat_id_water) {
  if (in_bounds(x, y + 1) && !is_empty(x, y + 1)) {
    p->velocity.y /= 2.f;
  }
  ...
  // Simple falling, changing the velocity here ruins everything. I need to redo
  // this entire simulation.
...
void update_sand(u32 x, u32 y) {
  ...
  // Just check if you can move directly beneath you. If not, then reset your
  // velocity. God, this is going to blow.
  if (in_bounds(x, y + 1) && !is_empty(x, y + 1) &&
      get_particle_at(x, y + 1).id != mat_id_water) {
    p->velocity.y /= 2.f;
  }
  ...
  if (in_bounds(vi_x, vi_y) &&
      ((is_empty(vi_x, vi_y) ||
        (((g_world_particle_data[compute_idx(vi_x, vi_y)].id == mat_id_water) &&
          !g_world_particle_data[compute_idx(vi_x, vi_y)]
               .has_been_updated_this_frame &&
          gs_vec2_len(g_world_particle_data[compute_idx(vi_x, vi_y)].velocity) -
                  gs_vec2_len(tmp_a.velocity) >
              10.f))))) {
  ...
  // this entire simulation.
           ...
           ((is_empty(x, y + 1) ||
             (g_world_particle_data[b_idx].id == mat_id_water)))) {
    p->velocity.y += (gravity * dt);
    ...
    write_data(read_idx, tmp_b);
             ...
             ((is_empty(x - 1, y + 1) ||
               g_world_particle_data[bl_idx].id == mat_id_water))) {
    p->velocity.x = is_in_liquid(x, y, &lx, &ly) ? 0.f
                    : random_val(0, 1) == 0      ? -1.f
                                                 : 1.f;
    p->velocity.y += (gravity * dt);
    ...
    write_data(read_idx, tmp_b);
             ...
             ((is_empty(x + 1, y + 1) ||
               g_world_particle_data[br_idx].id == mat_id_water))) {
    p->velocity.x = is_in_liquid(x, y, &lx, &ly) ? 0.f
                    : random_val(0, 1) == 0      ? -1.f
                                                 : 1.f;
    p->velocity.y += (gravity * dt);
...
void update_steam(u32 x, u32 y) {
  ...
  p->velocity.x =
      gs_clamp(p->velocity.x + (f32)random_val(-100, 100) / 100.f, -1.f, 1.f);

  // Change color based on life_time
  p->color.r =
      (u8)(gs_clamp((gs_interp_linear(0.f, 10.f, p->life_time) / 10.f) * 255.f,
                    150.f, 255.f));
  p->color.g =
      (u8)(gs_clamp((gs_interp_linear(0.f, 10.f, p->life_time) / 10.f) * 255.f,
                    150.f, 255.f));
  p->color.b =
      (u8)(gs_clamp((gs_interp_linear(0.f, 10.f, p->life_time) / 10.f) * 255.f,
                    150.f, 255.f));
  p->color.a = (u8)(gs_clamp(
      (gs_interp_linear(10.f, 0.f, p->life_time) / 10.f) * 255.f, 10.f, 255.f));
  ...
  // this entire simulation.
           ...
           ((is_empty(x, y - 1) ||
             (get_particle_at(x, y - 1).id == mat_id_water) ||
             get_particle_at(x, y - 1).id == mat_id_fire))) {
    p->velocity.y -= (gravity * dt);
    particle_t tmp_b = get_particle_at(x, y - 1);
    ...
    write_data(read_idx, tmp_b);
             ...
             ((is_empty(x - 1, y - 1) ||
               get_particle_at(x - 1, y - 1).id == mat_id_water) ||
              get_particle_at(x - 1, y - 1).id == mat_id_fire)) {
    p->velocity.x = random_val(0, 1) == 0 ? -1.2f : 1.2f;
    p->velocity.y -= (gravity * dt);
    particle_t tmp_b = get_particle_at(x - 1, y - 1);
  ...
  // Can move if in liquid
           ...
           (get_particle_at(x + 1, y).id == mat_id_water)) {
    u32 idx = compute_idx(x + 1, y);
    particle_t tmp_b = g_world_particle_data[idx];
    write_data(idx, *p);
    write_data(read_idx, tmp_b);
  } else if (in_bounds(x - 1, y) &&
             (g_world_particle_data[compute_idx(x - 1, y)].id ==
              mat_id_water)) {
    u32 idx = compute_idx(x - 1, y);
    particle_t tmp_b = g_world_particle_data[idx];
    write_data(idx, *p);
...
void update_smoke(u32 x, u32 y) {
  ...
  // Change color based on life_time
  p->color.r = (u8)(gs_clamp(
      (gs_interp_linear(10.f, 0.f, p->life_time * 0.5f) / 10.f) * 150.f, 0.f,
      150.f));
  p->color.g = (u8)(gs_clamp(
      (gs_interp_linear(10.f, 0.f, p->life_time * 0.5f) / 10.f) * 120.f, 0.f,
      120.f));
  p->color.b = (u8)(gs_clamp(
      (gs_interp_linear(10.f, 0.f, p->life_time * 0.5f) / 10.f) * 100.f, 0.f,
      100.f));
  ...
  // get_particle_at(vi_x, vi_y).id == mat_id_water || get_particle_at(vi_x,
  // vi_y).id == mat_id_fire))) {
  ...
  // this entire simulation.
           ...
           get_particle_at(x, y - 1).id != mat_id_smoke &&
    ...
    write_data(read_idx, tmp_b);
             ...
             get_particle_at(x - 1, y - 1).id != mat_id_smoke &&
  ...
  // Can move if in liquid
           ...
           get_particle_at(x + 1, y).id != mat_id_smoke &&
...
void update_ember(u32 x, u32 y) {
  ...
  if (in_bounds(x, y + 1) && get_particle_at(x, y + 1).id == mat_id_wood &&
      random_val(0, 200) == 0) {
  ...
  if (in_bounds(vi_x, vi_y) &&
      (is_empty(vi_x, vi_y) || get_particle_at(vi_x, vi_y).id == mat_id_water ||
       get_particle_at(vi_x, vi_y).id == mat_id_fire ||
  ...
  // Can move if in liquid
           ...
           (is_empty(x + 1, y) ||
            get_particle_at(x + 1, y).id == mat_id_fire)) {
    u32 idx = compute_idx(x + 1, y);
    particle_t tmp_b = g_world_particle_data[idx];
    write_data(idx, *p);
    write_data(read_idx, tmp_b);
...
void update_fire(u32 x, u32 y) {
  ...
  f32 st = sin(gs_platform_elapsed_time());
  ...
  p->velocity.x =
      gs_clamp(p->velocity.x + (f32)random_val(-100, 100) / 200.f, -0.5f, 0.5f);
  ...
  if (random_val(0, (s32)(p->life_time * 100.f)) % 200 == 0) {
    ...
    switch (ran) {
    case 0:
      p->color = (color_t){255, 80, 20, 255};
      break;
    case 1:
      p->color = (color_t){250, 150, 10, 255};
      break;
    case 2:
      p->color = (color_t){200, 150, 0, 255};
      break;
    case 3:
      p->color = (color_t){100, 50, 2, 255};
      break;
    }
  }
  ...
  if (random_val(0, 10) == 0) {
    // p->velocity.x = gs_clamp(p->velocity.x + (f32)random_val(-1, 1) / 2.f,
    // -1.f, 1.f);
  }
  // p->velocity.x = gs_clamp(p->velocity.x, -0.5f, 0.5f);

  // Kill fire underneath
  if (in_bounds(x, y + 3) && get_particle_at(x, y + 3).id == mat_id_fire &&
      random_val(0, 100) == 0) {
  ...
  if (in_bounds(x, y + 1) && get_particle_at(x, y + 1).id == mat_id_fire &&
    ...
    if (random_val(0, 10) == 0 * p->life_time < 10.f && p->life_time > 1.f) {
      ...
      s32 spread = 3;
      for (s32 i = rh; i < 0; ++i) {
        for (s32 j = r ? -spread : spread; r ? j < spread : j > -spread;
             r ? ++j : --j) {
          s32 rx = j, ry = i;
  ...
  if (random_val(0, 250) == 0 && p->life_time < 3.f) {
    for (u32 i = 0; i < random_val(1, 100); ++i) {
      if (in_bounds(x, y - 1) && is_empty(x, y - 1)) {
        ...
        e.velocity = (gs_vec2){(f32)random_val(-5, 5) / 5.f,
                               -(f32)random_val(2, 10) / 10.f};
  ...
  if (in_bounds(x, y + 1) &&
      ((get_particle_at(x, y + 1).id == mat_id_wood &&
        random_val(0, wood_chance) == 0) ||
       (get_particle_at(x, y + 1).id == mat_id_gunpowder &&
        random_val(0, gun_powder_chance) == 0) ||
       (get_particle_at(x, y + 1).id == mat_id_oil &&
        random_val(0, oil_chance) == 0)
        ...
        for (s32 j = r ? -3 : 2; r ? j < 2 : j > -3; r ? ++j : --j) {
          ...
          if (in_bounds(x + rx, y + ry) && is_empty(x + rx, y + ry)) {
            ...
            break;
             ...
             ((get_particle_at(x + 1, y + 1).id == mat_id_wood &&
               random_val(0, wood_chance) == 0) ||
              (get_particle_at(x + 1, y + 1).id == mat_id_gunpowder &&
               random_val(0, gun_powder_chance) == 0) ||
              (get_particle_at(x + 1, y + 1).id == mat_id_oil &&
               random_val(0, oil_chance) == 0)
        ...
        for (s32 j = r ? -3 : 2; r ? j < 2 : j > -3; r ? ++j : --j) {
          ...
          if (in_bounds(x + rx, y + ry) && is_empty(x + rx, y + ry)) {
            ...
            break;
             ...
             ((get_particle_at(x - 1, y).id == mat_id_wood &&
               random_val(0, wood_chance) == 0) ||
              (get_particle_at(x - 1, y).id == mat_id_gunpowder &&
               random_val(0, gun_powder_chance) == 0) ||
              (get_particle_at(x - 1, y).id == mat_id_oil &&
               random_val(0, oil_chance) == 0)
  ...
  if (in_bounds(vi_x, vi_y) &&
      (is_empty(vi_x, vi_y) || get_particle_at(vi_x, vi_y).id == mat_id_fire ||
       get_particle_at(vi_x, vi_y).id == mat_id_smoke)) {
    // p->velocity.y -= (gravity * dt);
    particle_t tmp_b = g_world_particle_data[compute_idx(vi_x, vi_y)];
    write_data(compute_idx(vi_x, vi_y), *p);
  ...
  // this entire simulation.
    ...
    write_data(read_idx, tmp_b);
             ...
             (g_world_particle_data[compute_idx(x - 1, y - 1)].id ==
              mat_id_water)) {
    u32 idx = compute_idx(x - 1, y - 1);
    // p->velocity.x = random_val(0, 1) == 0 ? -1.f : 1.f;
    particle_t tmp_b = g_world_particle_data[idx];
    ...
    write_data(read_idx, tmp_b);
             ...
             (g_world_particle_data[compute_idx(x, y - 1)].id ==
              mat_id_water)) {
    u32 idx = compute_idx(x, y - 1);
    // p->velocity.x = random_val(0, 1) == 0 ? -1.f : 1.f;
    particle_t tmp_b = g_world_particle_data[idx];
...
void update_lava(u32 x, u32 y) {
  ...
  if (random_val(0, (s32)(p->life_time * 100.f)) % 200 == 0) {
    ...
    case 0:
      ...
      break;
    case 1:
      p->color = (color_t){255, 100, 10, 255};
      break;
    case 2:
      p->color = (color_t){255, 50, 0, 255};
      break;
    case 3:
      p->color = (color_t){200, 50, 2, 255};
      break;
    }
  }
  ...
  // s32 lx, ly;
  // if (is_in_liquid(x, y, &lx, &ly) && in_bounds(x, y - 1) &&
  // get_particle_at(x, y - 1).id == mat_id_water) { 	particle_t tmp =
  // get_particle_at(x, y - 1); 	write_data(compute_idx(x, y - 1), *p);
...
void update_oil(u32 x, u32 y) {
  ...
  // Change color depending on pressure? Pressure would dictate how "deep" the
  // water is, I suppose.
...
void update_acid(u32 x, u32 y) {
  ...
  if (in_bounds(x, y + 1) && ((get_particle_at(x, y + 1).id == mat_id_wood &&
                               random_val(0, wood_chance) == 0) ||
                              (get_particle_at(x, y + 1).id == mat_id_stone &&
                               random_val(0, stone_chance) == 0) ||
                              (get_particle_at(x, y + 1).id == mat_id_sand &&
                               random_val(0, sand_chance) == 0) ||
                              (get_particle_at(x, y + 1).id == mat_id_salt &&
                               random_val(0, salt_chance) == 0)
    ...
    write_data(read_idx, particle_empty());
             ...
             ((get_particle_at(x + 1, y + 1).id == mat_id_wood &&
              ...
              (get_particle_at(x + 1, y + 1).id == mat_id_stone &&
               random_val(0, stone_chance) == 0) ||
              (get_particle_at(x + 1, y + 1).id == mat_id_sand &&
               random_val(0, sand_chance) == 0) ||
              (get_particle_at(x + 1, y + 1).id == mat_id_salt &&
               random_val(0, salt_chance) == 0)
    ...
    write_data(read_idx, particle_empty());
             ...
             ((get_particle_at(x - 1, y).id == mat_id_wood &&
              ...
              (get_particle_at(x - 1, y).id == mat_id_stone &&
               random_val(0, stone_chance) == 0) ||
              (get_particle_at(x - 1, y).id == mat_id_sand &&
               random_val(0, sand_chance) == 0) ||
              (get_particle_at(x - 1, y).id == mat_id_salt &&
               random_val(0, salt_chance) == 0)
  ...
  // this entire simulation.
    ...
    if (completely_surrounded(x, y)) {
      ...
      for (u32 i = 0; i < fall_rate; ++i) {
        for (s32 j = spread_rate; j > 0; --j) {
          if (in_bounds(x - j, y + i) &&
              (is_empty(x - j, y + i) ||
               get_particle_at(x - j, y + i).id == mat_id_oil)) {
...
void update_water(u32 x, u32 y) {
  ...
  // Simple falling, changing the velocity here ruins everything. I need to redo
  // this entire simulation.
  else if (in_bounds(x, y + u) &&
           ((is_empty(x, y + u) ||
             (g_world_particle_data[b_idx].id == mat_id_oil)))) {
    p->velocity.y += (gravity * dt);
    particle_t tmp_b = get_particle_at(x, y + u);
    write_data(b_idx, *p);
    write_data(read_idx, tmp_b);
  } else if (in_bounds(x + l, y + u) &&
             ((is_empty(x + l, y + u) ||
               g_world_particle_data[bl_idx].id == mat_id_oil))) {
    p->velocity.x = is_in_liquid(x, y, &lx, &ly) ? 0.f
                    : random_val(0, 1) == 0      ? -1.f
                                                 : 1.f;
    p->velocity.y += (gravity * dt);
    ...
    write_data(bl_idx, *p);
    write_data(read_idx, tmp_b);
  } else if (in_bounds(x + r, y + u) &&
             ((is_empty(x + r, y + u) ||
               g_world_particle_data[br_idx].id == mat_id_oil))) {
    p->velocity.x = is_in_liquid(x, y, &lx, &ly) ? 0.f
                    : random_val(0, 1) == 0      ? -1.f
                                                 : 1.f;
    p->velocity.y += (gravity * dt);
